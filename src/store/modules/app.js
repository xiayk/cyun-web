...
const app = {
    ...
    mutations: {
        ...
        //                     if (child.access !== undefined) {
        //                         if (
        //                             Util.showThisRoute(child.access, accessCode)
        //                         ) {
        //                             return child;
        //                         }
        //                     } else {
        ...
        async updateMenulist(state) {
            ...
            if (token) {
                ...
                if (accessMenuDefer.data.length > 0) {
                    accessMenuDefer.data.forEach((item, index) => {
                        ...
                        state.accessRouter.push(item.menuCode);
                        if (item.childrens.length > 0) {
                            item.childrens.forEach(item2 => {
                                state.accessRouter.push(item2.code);
                                // accessMenu.push(item2.code);
                                let subMenu = {
                                    path: "/" + item2.menuUrl,
                                    name: item2.menuCode,
                                ...
                                accessAppRouter[index].children.push(subMenu);
                                // item2.functions.forEach(fun => {
                                //     state.accessRouter.push(fun.code);
                                // });
                            });
                        }
                    });
            ...
            accessAppRouter.forEach((item, index) => {
                if (item.access !== undefined) {
                    if (Util.showThisRoute(item.access, accessCode)) {
                        if (item.children.length === 1) {
                            menuList.push(item);
