    ...
    <div>
        <Card class="filter-wrap">
            <Form @submit.native.prevent="handleFilter" :model="filter" ref="filterForm" label-position="right" :label-width="120" >
                ...
                <FormItem label="支付通道">
                    <institution-list-selector  v-model="filter.institutionId" mode="id"></institution-list-selector>
                </FormItem>
...
import { getSettleInstitutionStat } from "@/actions/settle";
export default {
    name: "transaction-institution-order-stat",
    data() {
        return {
            loading: false,
            columns: [
    ...
    methods: {
        loadData() {
            this.loading = true;
            getSettleInstitutionStat(this.filter).then(
                res => {
                    this.loading = false;
        ...
        downloadExcel() {
            this.$util.downloadExcel(
                "/settle/statistics/export/institution/list",
                this.filter
            );
        }
    },
