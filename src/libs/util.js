...
util.recursivePluck = function(arr, column, subNode) {
    let ret = [];
    arr.forEach(item => {
        ret.push(item[column]);
        if (item[subNode]) {
            ret.push(...util.recursivePluck(item[subNode], column, subNode));
        }
    });
    return ret;
};
 ...
 */
util.encodeSearchParams = function(obj) {
    const params = [];
    Object.keys(obj).forEach(key => {
        if (key === "limit" || key === "offset" || key === "_dateRange") {
            return;
        }
        let value = obj[key];
        // 如果值为undefined我们将其置空
...
util.downloadExcel = function(apiUrl, filter) {
    util.downloadByForm(apiUrl, filter);
};
util.downloadByForm = function(apiUrl, params, method) {
    ...
    for (var key in params) {
        if (key != "limit"&& key != "offset"&&!/^_/.test(key)) {
